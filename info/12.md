...
모듈명|설명|상태|소스파일|지원
---|---|---|---|---
auth_basic_module|Basic authentication|Base|mod_auth_basic.c|아파치 2.1 이후부터
authn_file_module|문자파일을 이용한 사용자 인증|Base|mod_authn_file.c|아파치 2.1 이후부터
authz_groupfile_module|일반 문자파일을 이용한 그룹 권한부여|Base|mod_authz_groupfile.c|아파치 2.1 이후부터
authz_host_module|호스트 (이름이나 IP 주소)를 사용한 그룹 권한부여|Base|mod_authz_host.c|아파치 2.1 이후부터
