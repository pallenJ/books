...
모듈|설명|문법
---|---|---
mod_auth_basic|인증과 권한부여를<br> 저수준 모듈에 넘겨줄지 결정한다|AuthBasicAuthoritative On/Off
mod_authn_file|인증할 사용자명와 암호 목록을 저장하는 문자파일명을 지정한다|AuthUserFile file-path
mod_authz_groupfile|인증에 사용할 사용자 그룹 목록을 저장하는 문자파일명을 지정한다|AuthGroupFile file-path
mod_authz_host|서버의 일부에 접근할 수 있는 호스트를 지정한다| Allow from alll / \|hostenv=env-variable [host/env=env-variable] ...<br>_실제론 /대신 shift+\를 써야함_
